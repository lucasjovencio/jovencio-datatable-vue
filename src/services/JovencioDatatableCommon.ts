    ...
    static  providerButtonDT(provider:JovencioButtonProvider) {
		...
		for (let button of buttons.filter((row:any) => row.enable)) {
			...
			if (button.key === 'lock' && button.enable) {
				...
				icon = (button.html_icon) ? button.html_icon : svg;
				...
				>
					<span id="${provider.meta.row}-${provider.meta.col}-lock-${idClass}"> ${icon} ${name}</span>
				...
				svg = (button.svg) ? button.svg : (button.name ? button.name : '');
				icon = (button.html_icon) ? button.html_icon : svg;
