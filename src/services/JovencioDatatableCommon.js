...
class JovencioDatatableCommon {
    static  providerButtonDT(provider, context = null) {
		...
		for (let button of buttons.filter(row => row.enable)) {
    ...
    static providerClousureDT(selector, trigger, triggerSignature, lock, enable = true, type = 'action') {
		if (type == 'action') {
			const actionClousure = function (keyClass) {
				return { 
			...
			const lockClousure = function(keyClass) {
				return { 
					'selector': selector+"-"+keyClass,
					'clousure': (self) => {
